...
module.exports = function ForgotPasswordControllerModule(pb) {
    ...
    ForgotPasswordController.prototype.verifyCaptcha = function(returnURL, ip, secret, post, cb) {
        var self = this;
        request({
            method: 'POST',
            ...
            if (error || !body.success) {
                return self.formError(self.ls.get('INVALID_LOGIN_RECAPTCHA'), returnURL, cb);
            }
            self.forgotPassword(returnURL, post, cb);
        });
    };
    ...
    ForgotPasswordController.prototype.onPostParamsRetrieved = function(post, cb) {
        ...
        pluginService.getSettingsKV('google-recaptcha', function(err, settings) {
            ...
            var returnURL = post.adminAttempt ? '/admin/login' : '/user/login';
            if ((settings.require_recaptcha_for_admin_login && post.adminAttempt) || (settings.require_recaptcha_for_user_login && !post.adminAttempt)) {
                self.verifyCaptcha(returnURL, ip, settings.google_recaptcha_secret_key, post, cb);
            }
            else {
                self.forgotPassword(returnURL, post, cb);
            }
    ...
    ForgotPasswordController.prototype.forgotPassword = function(returnURL, post, cb) {
        var self = this;
        ...
        dao.loadByValues(query, 'user', function(err, user) {
            ...
            pb.settings.get('email_settings', function(err, emailSettings) {
                ...
                dao.loadByValue('user_id', userIdStr, 'password_reset', function(err, passwordReset) {
                    ...
                    if (!passwordReset) {
                        passwordReset = pb.DocumentCreator.create('password_reset', {
                            user_id: userIdStr
                        });
                    }
    ...
    ForgotPasswordController.getRoutes = function(cb) {
        var routes = [{
            method: 'post',
